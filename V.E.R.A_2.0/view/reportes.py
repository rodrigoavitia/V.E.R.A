...
class ReportesView(ctk.CTkFrame):
    def __init__(self, master, controller):
        ...
        # ==========================================
        # 🚀 BANNER FASE BETA (VERDE) 🚀
        # ==========================================
        self.banner = ctk.CTkFrame(self.main_card, fg_color="#10B981", height=40, corner_radius=0) # Verde Éxito
        self.banner.pack(fill="x", side="top")
        
        ctk.CTkLabel(
            self.banner, 
            text="🚀 FASE BETA - SISTEMA DE REPORTES ACTIVO", 
            text_color="white", 
            font=("Arial", 14, "bold")
        ...
        ctk.CTkButton(row2, text="📅", width=30, fg_color="#E5E7EB", text_color="black", hover_color="#D1D5DB", 
                      command=lambda: self.abrir_calendario(self.entry_fecha_ini)).pack(side="left", padx=(2, 15))
        ...
        ctk.CTkButton(row2, text="📅", width=30, fg_color="#E5E7EB", text_color="black", hover_color="#D1D5DB", 
                      command=lambda: self.abrir_calendario(self.entry_fecha_fin)).pack(side="left", padx=(2, 20))

        # Botones de Acción
        ctk.CTkButton(row2, text="🔍 Filtrar Resultados", width=150, fg_color="#0092B8", command=lambda: self.realizar_busqueda(None)).pack(side="left")
    ...
    def realizar_busqueda(self, event):
        ...
        for fila_datos in resultados: self.crear_fila(fila_datos)
    ...
    def ver_detalle(self, id_reporte):
        ...
        try:
            ...
            ctk_img = ctk.CTkImage(light_image=Image.open(image_data), size=(380, 280))
